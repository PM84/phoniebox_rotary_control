...
echo -e "///////////////////////////////////////////////////////////////////////////////////////"
echo -e ""
echo -e "Do you want to install or remove Rotary-Control-Service for phoniebox?"
echo -e " "
...
sudo rm /etc/systemd/system/phoniebox-rotary-control.service > /dev/null 2>&1
...
do
    case $opt in
        "Start")
			echo -e " "
            sudo service phoniebox-rotary-control start > /dev/null 2>&1
            pid=`ps -ef | grep rotary_volume_control.py | grep -v grep | awk '{print $2}'`  > /dev/null 2>&1
			if [ -z "${pid}" ]
			then
